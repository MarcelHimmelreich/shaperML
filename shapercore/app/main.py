...
@app.get("/")
async def read_root():
    message = f"Hello world! From FastAPI running on Uvicorn with Gunicorn in Alpine."
    return {"message": message}


@app.put("/metric/accuracy/")
async def accuracy(true: str, predict: str):
    return


@app.put("/metric/f1/")
def f1_score(true: str, predict: str, average: str ="weighted"):
    return


@app.put("/metric/r2/")
def r2_score(true: str, predict: str):
    return


@app.put("/metric/recall/")
def recall_score(true: str, predict: str):
    return


@app.put("/metric/precision/")
def precision_score(true: str, predict: str):
    return


#Set Estimator & Save Model to Minio
@app.put("/model/fit/")
def fit(truth: str):
    return


#Get Model from Minio & Predict
@app.put("/model/predict/")
def predict(predict: bytearray):
    return


@app.put("/model/crossvalidation/")
def cross_validation(estimator: str, k_fold: int):
    return
